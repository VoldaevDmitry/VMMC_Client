...
namespace VMMC_Core
{
    public class LocalFile : INotifyPropertyChanged
    {
        ...
        private VMMC_Core.Relationship relationship;
        private VMMC_Core.Revision revision;
        private VMMC_Core.Document document;
        private VMMC_Core.Complekt complekt;
        ...
        public VMMC_Core.Relationship Relationship
        {
        ...
        public VMMC_Core.Complekt Complekt
        {
            get { return complekt; }
            set
            {
        ...
        public string CheckForDublicatesInList(ObservableCollection<VMMC_Core.LocalFile> filesCollection)
        {
            string statusInfo = "";
            if (Checksum.Length == 32)
            {
                IEnumerable<VMMC_Core.LocalFile> dublicates = filesCollection.Where(x => x.Checksum == Checksum && x.LocalFilePath != LocalFilePath);
        ...
        public string CheckForDublicatesInDataBase(ObservableCollection<VMMC_Core.Files> dbFilesCollection)
        {
            string statusInfo = "";
            if (Checksum.Length == 32)
            {
                
                IEnumerable<VMMC_Core.Files> dublicates = dbFilesCollection.Where(x => x.Checksum == Checksum);
        ...
        private string CheckForDublicatesInDbDocument(VMMC_Core.SessionInfo session)
        {
            string statusInfo = "";
            //VMMC_Core.Document doc = getDocument(targetFile.Document.DocumentCode);
            VMMC_Core.Document doc = new VMMC_Core.Document(session).GetDocument(Document.DocumentCode);
            VMMC_Core.Revision rev = new VMMC_Core.Revision(session).GetRevision(doc.DocumentId, Revision.Number);


            if (Checksum.Length == 32)
            {
                ObservableCollection<VMMC_Core.Files> dbFilesCollection = new VMMC_Core.Files(session).GetFilesByRevision(rev.RevisionId);

                IEnumerable<VMMC_Core.Files> dublicates = dbFilesCollection.Where(x => x.Checksum != Checksum && x.FileName == LocalFileName);
